...
void displeyCompletedInfo(CompletedInfo completed, char *completed_info)
{
...
int checkBuiltinCmd(Argument arg, CompletedInfo *completed)
{
    ...
    {
        printf("Called set\n");
        // if (setEnvVar(argv + 0) != FAIL)
        // {
        //     return_value[(*return_size)++] = 1;
        //     return FAIL;
        // }
        // return_value[(*return_size)++] = 0;
        return SUCCESS;
    }
    ...
    if (strcmp(arg.arg_list[0], "exit") == 0)
    {
        fprintf(stderr, "Bye...\n");
        completed->return_value[(completed->return_size)++] = 0;
        displeyCompletedInfo(*completed, "exit");
        exit(0);
    }
...
int pipeCmd(char *curr_cmd[], int des[2], int file_des)
{
...
int redirCmd(char *curr_cmd[], char *next_cmd[], int file_des)
{
...
int multiCmd(char *curr_cmd[], char *next_cmd[], int curr_meta, Argument arg, CompletedInfo *completed, int file_des)
{
    ...
    if (pid == 0)
    {
        ...
        if (strcmp(arg.meta_list[curr_meta], "|") == 0)
        {
            pipeCmd(curr_cmd, des, file_des);
        }

        if (strcmp(arg.meta_list[curr_meta], ">") == 0)
        {
            redirCmd(curr_cmd, next_cmd, file_des);
        }
...
int main()
{
    ...
    while (1)
    {
        ...
        /* main */
        fprintf(stderr, "sshell@ucd$ "); // output prompt
        ...
        displeyCompletedInfo(completed, bak_command);
    }
    return 0;
}
